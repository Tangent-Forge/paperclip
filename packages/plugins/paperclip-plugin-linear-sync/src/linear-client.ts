...
export function createLinearClient(input: { http: PluginHttpClient; url: string; token: string }): LinearClient {
  return {
    async listCandidateIssues({ stateNames, first, updatedAfter }) {
      // Linear's filter field uses DateTimeOrDuration, not DateTime.
      // When updatedAfter is null (first run) we omit the updatedAt filter entirely.
      const updatedAtFilter = updatedAfter ? `updatedAt: { gt: $updatedAfter }` : "";
      const afterVar = updatedAfter ? ", $updatedAfter: DateTimeOrDuration" : "";
      const data = await graphql<{
        ...
        input.url,
        input.token,
        `query PaperclipCandidateIssues($stateNames: [String!], $first: Int!${afterVar}) {
          issues(
            first: $first,
            orderBy: updatedAt,
            filter: {
        ...
        updatedAfter ? { stateNames, first, updatedAfter } : { stateNames, first },
      );
