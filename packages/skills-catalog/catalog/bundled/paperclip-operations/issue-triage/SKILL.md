...
5. **Escalate** — the issue needs board, CTO, or user input. Create a `request_confirmation` / `ask_user_questions` with complete `payload.ownerGuidance`, or `request_board_approval` for formal spend/policy gates, and set the issue to `in_review`. Prefer board-seat/system resolution for soft_human before escalating. Do not escalate agent-ops or informational owner terminals as bare Decide cards.
