...
for pattern in "${PATTERNS[@]}"; do
  matches=$(grep -rl --include="*.ts" --include="*.js" --include="*.py" \
  ...
  if [[ -n "$matches" ]]; then
    echo "  [MATCH] Credential pattern detected"
    match_count=$(printf '%s\n' "$matches" | sed '/^$/d' | wc -l | tr -d ' ')
    echo "    Files: $match_count"
    # Print file names only. Never copy matching lines or credential values
    # into adapter stdout, run logs, or audit reports.
    { printf '%s\n' "$matches" | head -5 | while read -r file; do echo "    $file"; done; } 2>/dev/null || true
