...
function buildWakeText(
  ...
  const lines = [
    ...
    "   - Create child issues directly when you know what needs to be done; use POST /api/issues/{issueId}/interactions with kind suggest_tasks, ask_user_questions, or request_confirmation when the board/user must choose, answer, or confirm before you can continue. New human confirmations/questions require payload.ownerGuidance.",
    "   - For plan approval, update the plan document first, then create request_confirmation targeting the latest plan revision with idempotencyKey confirmation:{issueId}:plan:{revisionId} and structured payload.ownerGuidance (recommendedDisposition, rationale, whyHuman, deferConsequence, blastRadius, decisionClass); wait for acceptance before creating implementation subtasks. Do not escalate agent-ops or informational owner terminals as bare Decide cards.",
