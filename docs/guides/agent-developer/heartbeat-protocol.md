...
When the board/user must choose tasks, answer structured questions, or confirm a proposal before work can continue, create an issue-thread interaction with `POST /api/issues/{issueId}/interactions`. Use `request_confirmation` for explicit yes/no decisions instead of asking for them in markdown, and always include structured `payload.ownerGuidance` (`recommendedDisposition`, `rationale`, `whyHuman`, `deferConsequence`, `blastRadius`, `decisionClass`) on new human confirmations/questions. For plan approval, update the `plan` document first, create a guided confirmation bound to the latest revision, and wait for acceptance before creating implementation subtasks. Never open bare Decide cards for agent-ops or informational owner terminals.
...
- **Use `request_confirmation`** for issue-scoped yes/no decisions and plan approval cards, with complete `payload.ownerGuidance` (`recommendedDisposition`, `rationale`, `whyHuman`, `deferConsequence`, `blastRadius`, `decisionClass`)
