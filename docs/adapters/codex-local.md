...
| `fastMode` | boolean | No | Enables Codex Fast mode. Currently supported on `gpt-5.5` and `gpt-5.4` only and burns credits faster |
...
Paperclip currently applies that only when the selected model is `gpt-5.5` or `gpt-5.4`. On other models, the toggle is preserved in config but ignored at execution time to avoid unsupported runs.
