...
The image sets `GEMINI_SANDBOX=false` so the Gemini CLI does not try to launch its own (Docker-in-Docker) sandbox inside the container. The `gemini_local` adapter passes the appropriate no-sandbox flags for the selected CLI; for `agy`, it omits `--sandbox` unless sandbox mode is enabled. This env var only matters if you invoke a CLI manually inside the container; override it if you have nested-container support and want CLI-level sandboxing.
