...
describe("janitor_local execute approval gate", () => {
  ...
  it("forbids active cleanup when the approval gate is disabled", async () => {
    const root = await makeTempRoot();
    const result = await execute(makeCtx(root, {
      config: { cwd: root, modules: ["storage"], dryRun: false, approvalRequired: false },
    }));

    expect(result.exitCode).toBe(1);
    expect(result.errorCode).toBe("janitor_approval_bypass_forbidden");
    expect(runJanitorModuleMock).not.toHaveBeenCalled();
  });
  ...
  it.each([
    { name: "agent", agentId: "99999999-9999-4999-8999-999999999999", issueId: "33333333-3333-4333-8333-333333333333", cwd: null },
    { name: "issue", agentId: "11111111-1111-4111-8111-111111111111", issueId: "99999999-9999-4999-8999-999999999999", cwd: null },
    { name: "workspace", agentId: "11111111-1111-4111-8111-111111111111", issueId: "33333333-3333-4333-8333-333333333333", cwd: "other" },
  ])("rejects an approved cleanup scoped to another $name", async ({ agentId, issueId, cwd: cwdVariant }) => {
    const root = await makeTempRoot();
    const approvalCwd = cwdVariant ? path.join(root, cwdVariant) : root;
    installFetch((url) => {
      if (url.pathname === "/api/approvals/44444444-4444-4444-8444-444444444444") {
        return {
          id: "44444444-4444-4444-8444-444444444444",
          ...
          payload: {
            janitorLocalApproval: {
              ...
              runId: "run-original",
              cwd: approvalCwd,
              reportPath: path.join(approvalCwd, ".janitor", "reports", "audit-original.md"),
              modules: ["storage"],
              actionCount: 1,
              actions: [{ module: "storage", action: "delete", target: "tmp/cache", description: "Remove old cache" }],
            },
  ...
  it("redacts credential-shaped module output before writing a report", async () => {
    const root = await makeTempRoot();
    const credential = ["AK", "IA", "A".repeat(16)].join("");
    runJanitorModuleMock.mockResolvedValueOnce(moduleResult("storage", `candidate ${credential}\n`));

    const result = await execute(makeCtx(root));

    expect(result.exitCode).toBe(0);
    const reportDir = path.join(root, ".janitor", "reports");
    const reportName = (await fs.readdir(reportDir)).find((file) => file.startsWith("audit-"));
    expect(reportName).toBeTruthy();
    const report = await fs.readFile(path.join(reportDir, reportName!), "utf8");
    expect(report).not.toContain(credential);
    expect(report).toContain("***REDACTED***");
  });

  it("refuses to write reports outside the configured workspace", async () => {
    const root = await makeTempRoot();
    const result = await execute(makeCtx(root, {
      config: {
        cwd: root,
        reportDir: path.join(os.tmpdir(), "outside-janitor-report"),
        modules: ["storage"],
        dryRun: true,
        approvalRequired: true,
      },
    ...
    expect(result.exitCode).toBe(1);
    expect(result.errorCode).toBe("janitor_report_path_outside_workspace");
    expect(runJanitorModuleMock).not.toHaveBeenCalled();
  });
});
