...
find "$CWD" -maxdepth 5 -name "package.json" \
  ...
  metadata=$(node -e 'try{const fs=require("node:fs");const p=JSON.parse(fs.readFileSync(process.argv[1],"utf8"));process.stdout.write(String(p.name||"")+"\t"+String(p.version||""))}catch{}' "$f" 2>/dev/null || true)
  name=${metadata%%$'\t'*}
  ver=${metadata#*$'\t'}
  if [[ -n "$name" ]]; then
    echo "  $name@$ver  ($f)"
