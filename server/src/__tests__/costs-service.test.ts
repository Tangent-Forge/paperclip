...
describeEmbeddedPostgres("cost and finance aggregate overflow handling", () => {
  ...
  it("surfaces unpricedMeteredRunCount on summary/byAgent/byProvider/byBiller/byAgentModel so a $0 metered run is distinguishable from a genuinely free subscription run (TAN-709)", async () => {
    ...
    await db.insert(costEvents).values([
      // unpriced metered run (the TAN-709 bug shape): costCents 0 but billingType metered_api
      {
