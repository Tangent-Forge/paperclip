...
- For plan approval, update the `plan` document first, create `request_confirmation` targeting the latest `plan` revision with structured `payload.ownerGuidance` (recommendedDisposition, rationale, whyHuman, deferConsequence, blastRadius, decisionClass), use an idempotency key like `confirmation:{issueId}:plan:{revisionId}`, set the source issue to `in_review`, and do not create implementation subtasks until the board/user accepts it. Never escalate agent-ops or informational owner terminals as bare Decide cards.
