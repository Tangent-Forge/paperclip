...
The actual worker-host boundary test `server/src/__tests__/plugin-company-scope-integration.test.ts` now passes `1` file / `2` tests with the host process profile: it starts the real Linear and Council workers, exercises scheduled/webhook dispatch, and verifies host-issued scope propagation plus fail-closed mismatch/missing-scope behavior. The route-level host contract, plugin-level scoped service tests, and existing worker-manager mediation tests provide the complementary coverage. No secret values are logged or included in the scope assertions. No production process, database, deployment, or migration was touched.
