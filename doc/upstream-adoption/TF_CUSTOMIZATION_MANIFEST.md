...
| Linear Sync | `packages/plugins/paperclip-plugin-linear-sync/**` | move TF behavior to plugin | Added as a plugin package; work contract is plugin-local, Triage admission is reconciled with current master, and scheduled/webhook/API/config paths use explicit company scope; 23 tests pass, including host-issued webhook scope and fail-closed negative cases |
| Council email intake | `packages/plugins/paperclip-plugin-council-email-intake/**` | move TF behavior to plugin | Added as a plugin package; 8 tests pass; explicit company-scoped config/secret access and deterministic qualified namespace migration; external side effects remain owner/deployment gated |
...
The remediation candidate remains source-merge-blocked until the exact
validation, commit, push, CI, and external-review gates in the PR remediation
