...
| Linear Sync | `packages/plugins/paperclip-plugin-linear-sync/**` | move TF behavior to plugin | Added as a plugin package; work contract is plugin-local, Triage admission is reconciled with current master, and scheduled/webhook/API/config paths use explicit company scope; 22 tests pass |
